...
impl From<FundModel> for Vec<FundField> {
    fn from(model: FundModel) -> Self {
        let mut res = vec![];
        // Raw Openfund fields
        if let Some(model) = model.raw_openfund {
            vec![
                (
        ...
        // Derived fields
        let is_raw_openfund = model.is_raw_openfund.unwrap_or(false);
        if !is_raw_openfund {
            //TODO
        }
        res
...
impl From<&ShareClassModel> for Vec<ShareClassField> {
    fn from(model: &ShareClassModel) -> Self {
        let mut res = vec![];
        // Raw Openfund fields
        if let Some(model) = model.raw_openfund.clone() {
            //TODO
            vec![
                // Essential

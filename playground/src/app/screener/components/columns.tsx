...
export const columns: ColumnDef<Product>[] = [
    {
        id: "address", cell: ({row}) => <img
            src="glam.svg"
            className="min-w-8 min-h-8 max-h-8 max-w-8"
            alt="Sparkle Icon"
        />,
    }, {
    accessorKey: "name", header: ({column}) => (<DataTableColumnHeader column={column} title="Name"/>), cell: ({row}) => <div className="w-[100px] truncate">{row.getValue("name")}</div>,
        enableSorting: false,
        enableHiding: false,
    },
    {
    ...
    },
    {
        ...
        cell: ({ row }) => <div className="w-[80px]">{row.getValue("inception")}</div>,
        enableSorting: true,
        enableHiding: true,
    },
    {

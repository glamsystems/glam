...
export default function Trade() {
    ...
    const onSubmit: SubmitHandler<TradeSchema> = (values, event) => {
        const nativeEvent = event as unknown as React.BaseSyntheticEvent & { nativeEvent: { submitter: HTMLElement } };
    ...
    return (
        <FormProvider {...form}>
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 w-1/2 mt-16">
                            ...
                            render={({field}) => (<FormItem className="w-1/2">
                                ...
                                <FormControl>
                                    <Select value={field.value} onValueChange={field.onChange}>
                                        ...
                                        <SelectContent>
                                            {tradeSchema.shape.venue._def.values.map(option => (<SelectItem key={option} value={option}>
                                                {option}
                                            </SelectItem>))}
                                        </SelectContent>
                                    </Select>
                            ...
                            render={({field}) => (<FormItem className="w-1/2">
                                ...
                                <FormControl>
                                    <Select value={field.value} onValueChange={field.onChange}>
                                        ...
                                        <SelectContent>
                                            {tradeSchema.shape.type._def.values.map(option => (<SelectItem key={option} value={option}>
                                                {option}
                                            </SelectItem>))}
                                        </SelectContent>
                                    </Select>
                    ...
                    <div className="flex space-x-4 items-center">
                        <AssetInput className="min-w-1/2 w-1/2" name="from" label="From" balance={100} selectedAsset={fromAsset} onSelectAsset={setFromAsset} />
                        <Button variant="outline" size="icon" onClick={(event) => handleFlip(event)} className="mt-1 min-w-10">
                            <ColumnSpacingIcon/>
                        </Button>
                        <FormField
                            control={form.control}
                        ...
                        />
                        <AssetInput className="min-w-1/2 w-1/2" name="to" label="To" balance={2000.12345} selectedAsset={toAsset} onSelectAsset={setToAsset} />                    </div>
                    <div  className="flex space-x-4 items-center">
                        <FormField
                            control={form.control}
                            name="exactMode"
                    ...
                    <div className="flex space-x-4 items-center">
                        <FormField
                            ...
                            render={({field}) => (<FormItem className="flex flex-row items-center space-x-3 space-y-0">
                                ...
                                <FormLabel htmlFor="direct-route-only" className="font-normal">
                                    Direct Route Only
                                </FormLabel>
                            </FormItem>)}
                        />
                        ...
                        <FormField
                            ...
                            render={({field}) => (<FormItem className="flex flex-row items-center space-x-3 space-y-0">
                                ...
                                <FormLabel htmlFor="versioned-transactions" className="font-normal">
                    ...
                    <div className="flex space-x-4 w-full">
                        <Button className="w-1/2" variant="ghost" onClick={(event) => handleClear(event)}>
                            Clear
                        </Button>
                        <Button className="w-1/2" type="submit">Swap</Button>
                    </div>
                </form>
            </Form>
        </FormProvider>);
